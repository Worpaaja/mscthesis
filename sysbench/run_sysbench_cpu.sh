...
do	
	...
	min_lat=$(awk -v  sum1="$min_lat" -v sum2="$(grep -i 'min' -F "$filename" | tr -d " " | cut -d ":" -f 2 )" 'BEGIN {printf "%.3f", sum1+sum2; exit(0)}' )
...
min_lat=$(awk -v  dividend="$min_lat" -v divisor="$1" 'BEGIN {printf "%.2f", dividend/divisor; exit(0)}' ) 
...
echo "Min latency average across all runs: $min_lat" >> benchmark_cpu/summary_$dirname.txt
